...
export const WIDE_AREA_DISCOVERY_DOMAIN = "hexos.internal.";
export const WIDE_AREA_ZONE_FILENAME = "hexos.internal.db";
...
function extractContentHash(zoneText) {
    const match = zoneText.match(/^\s*;\s*hexos-content-hash:\s*(\S+)\s*$/m);
    return match?.[1] ?? null;
}
...
function renderZone(opts) {
    const hostname = os.hostname().split(".")[0] ?? "hexos";
    const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "hexos");
    const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "hexos-gw");
    const txt = [
    ...
    records.push(`_hexos-gw._tcp IN PTR ${instanceLabel}._hexos-gw._tcp`);
    records.push(`${instanceLabel}._hexos-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
    records.push(`${instanceLabel}._hexos-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
    ...
    return `; hexos-content-hash: ${contentHash}\n${contentBody}`;
}
