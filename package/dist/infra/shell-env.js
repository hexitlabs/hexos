...
export function loadShellEnvFallback(opts) {
    ...
    try {
        ...
        logger.warn(`[hexos] shell env fallback failed: ${msg}`);
        lastAppliedKeys = [];
...
export function shouldEnableShellEnvFallback(env) {
    return isTruthyEnvValue(env.HEXOS_LOAD_SHELL_ENV);
}
export function shouldDeferShellEnvFallback(env) {
    return isTruthyEnvValue(env.HEXOS_DEFER_SHELL_ENV_FALLBACK);
}
export function resolveShellEnvFallbackTimeoutMs(env) {
    const raw = env.HEXOS_SHELL_ENV_TIMEOUT_MS?.trim();
    if (!raw)
