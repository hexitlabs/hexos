...
import { CONFIG_PATH_HEXOS } from "../config/config.js";
...
export async function removeChannelConfigWizard(cfg, runtime) {
    ...
    while (true) {
        ...
        if (configured.length === 0) {
            note([
                "No channel config found in hexos.json.",
                `Tip: \`${formatCliCommand("hexos channels status")}\` shows what is configured and enabled.`,
            ].join("\n"), "Remove channel");
            return next;
        }
        ...
        const confirmed = guardCancel(await confirm({
            message: `Delete ${label} configuration from ${shortenHomePath(CONFIG_PATH_HEXOS)}?`,
            initialValue: false,
        }), runtime);
        if (!confirmed)
            continue;
