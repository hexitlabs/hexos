...
import { scheduleGatewaySigusr1Restart, triggerHexOSRestart } from "../../infra/restart.js";
...
export const handleRestartCommand = async (params, allowTextCommands) => {
    ...
    if (hasSigusr1Listener) {
        ...
        return {
            shouldContinue: false,
            reply: {
                text: "⚙️ Restarting hexos in-process (SIGUSR1); back in a few seconds.",
            },
        };
    }
    const restartMethod = triggerHexOSRestart();
    if (!restartMethod.ok) {
    ...
    return {
        shouldContinue: false,
        reply: {
            text: `⚙️ Restarting hexos via ${restartMethod.method}; give me a few seconds to come back online.`,
        },
    };
};
