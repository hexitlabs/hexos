...
- This plugin integrates Lobster with HexOS *without core changes*.
...
## Using `clawd.invoke` (Lobster → HexOS tools)

Some Lobster pipelines may include a `clawd.invoke` step to call back into HexOS tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).

For this to work, the HexOS Gateway must expose the tool bridge endpoint and the target tool must be allowed by policy:

- HexOS provides an HTTP endpoint: `POST /tools/invoke`.
- The request is gated by **gateway auth** (e.g. `Authorization: Bearer …` when token auth is enabled).
- The invoked tool is gated by **tool policy** (global + per-agent + provider + group policy). If the tool is not allowed, HexOS returns `404 Tool not available`.
