...
export function listBlueBubblesAccountIds(cfg: HexOSConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: HexOSConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: HexOSConfig): ResolvedBlueBubblesAccount[] {
