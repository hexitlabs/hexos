...
export function listGoogleChatAccountIds(cfg: HexOSConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: HexOSConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: HexOSConfig): ResolvedGoogleChatAccount[] {
