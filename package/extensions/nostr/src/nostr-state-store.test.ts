...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.HEXOS_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "hexos-nostr-"));
  process.env.HEXOS_STATE_DIR = dir;
  setNostrRuntime({
    state: {
  ...
  try {
    ...
    if (previous === undefined) delete process.env.HEXOS_STATE_DIR;
    else process.env.HEXOS_STATE_DIR = previous;
