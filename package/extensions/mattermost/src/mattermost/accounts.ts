...
export function listMattermostAccountIds(cfg: HexOSConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultMattermostAccountId(cfg: HexOSConfig): string {
...
export function listEnabledMattermostAccounts(cfg: HexOSConfig): ResolvedMattermostAccount[] {
