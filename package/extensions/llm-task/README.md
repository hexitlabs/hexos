...
`clawd.invoke --each`) without adding new HexOS code per workflow.
...
This extension depends on HexOS internal modules (the embedded agent runner).
It is intended to ship as a **bundled** HexOS extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.hexos/extensions` as a standalone plugin directory.
