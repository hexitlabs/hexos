...
Recommended path: use the **CLI onboarding wizard** (`hexos onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `hexos configure --section web` (stores `tools.web.search.apiKey`).
...
- OAuth credentials (legacy import): `~/.hexos/credentials/oauth.json`
...
If you’re hacking on HexOS itself, run from source:
...
If `hexos health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `hexos status --all` is the best pasteable, read-only debug report.
Health probes: `hexos health` (or `hexos status --deep`) asks the running gateway for a health snapshot.
