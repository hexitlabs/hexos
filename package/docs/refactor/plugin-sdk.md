...
Delivery:
- Publish as `@hexos/plugin-sdk` (or export from core under `hexos/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `HexOSPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: HexOSConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: HexOSConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: HexOSConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: HexOSConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: HexOSConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: HexOSConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@hexos/plugin-sdk`.
- Add `api.runtime` to `HexOSPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `hexosRuntime: ">=2026.2.0"`).
