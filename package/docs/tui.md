...
Remote Gateway:
```bash
hexos tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `hexos tui --deliver`
...
- Check the Gateway logs: `hexos logs --follow`.
- Confirm the agent can run: `hexos status` and `hexos models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).
- `--history-limit <n>`: History entries to load (default 200)

## Troubleshooting
- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `hexos agents list` and your routing config.
