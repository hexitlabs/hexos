...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `hexos sandbox explain`.
...
Legacy `agent.*` configs are migrated by `hexos doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=hexos.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${HEXOS_STATE_DIR:-$HOME/.hexos}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
