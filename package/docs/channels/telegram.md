...
- If Telegram rejects the HTML payload, HexOS retries the same message as plain text.

## Commands (native + custom)
HexOS registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
Notes:
- Custom commands are **menu entries only**; HexOS does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `hexos logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.

## Config writes
By default, Telegram is allowed to write config updates triggered by channel events or `/config set|unset`.

This happens when:
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). HexOS can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. HexOS:
...
Private chats can include `message_thread_id` in some edge cases. HexOS keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
- Approve via:
  ...
  - `hexos pairing approve telegram <CODE>`
...
2) Run `hexos logs --follow` and look for `from.id`.
...
HexOS defaults to audio files for backward compatibility.
...
HexOS uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, HexOS:
...
**Requirements:**
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by HexOS)
...
- Example: `hexos message send --channel telegram --target 123456789 --message "hi"`.
...
- `hexos channels status` shows a warning when config expects unmentioned group messages.
- `hexos channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `hexos logs --follow` (look for "skipping group message")
...
- Upgrade to a HexOS build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
