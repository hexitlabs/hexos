...
```bash
hexos plugins install @hexos/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
hexos plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "HexOS" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `hexos pairing list nextcloud-talk`
  - `hexos pairing approve nextcloud-talk <CODE>`
