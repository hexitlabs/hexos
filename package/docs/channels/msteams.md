...
```bash
hexos plugins install @hexos/msteams
```

Local checkout (when running from a git repo):
```bash
hexos plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
HexOS will offer the local install path automatically.
...
3) Configure HexOS with those credentials.
...
## Goals
- Talk to HexOS via Teams DMs, group chats, or channels.
...
- On startup, HexOS resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.hexos/hexos.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring HexOS, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `hexos-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `hexos plugins install @hexos/msteams`
   - From a local checkout: `hexos plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "HexOS in Teams", "full": "HexOS in Teams" },
...
HexOS handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, HexOS only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/HexOSShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
HexOS sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `hexos message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.hexos/msteams-polls.json`.
...
**CLI:**
```bash
hexos message send --channel msteams \
...
hexos message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
hexos message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
hexos message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
hexos message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
