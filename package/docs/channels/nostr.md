...
read_when:
  - You want HexOS to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables HexOS to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`hexos onboard`) and `hexos channels add` list optional channel plugins.
...
```bash
hexos plugins install @hexos/nostr
```

Use a local checkout (dev workflows):

```bash
hexos plugins install --link <path-to-hexos>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "hexos@example.com",
        "lud16": "hexos@example.com"
      }
    }
  }
}
