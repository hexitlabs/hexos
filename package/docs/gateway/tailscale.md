...
HexOS can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. HexOS requires a shared password.
...
- `token` (default when `HEXOS_GATEWAY_TOKEN` is set)
- `password` (shared secret via `HEXOS_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
valid Serve proxy requests can authenticate via Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. HexOS only
...
Prefer `HEXOS_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
hexos gateway --tailscale serve
hexos gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want HexOS to undo `tailscale serve`
