...
HexOS features that can generate provider usage or paid API calls.
...
- `hexos status --usage` and `hexos channels list` show provider **usage windows**
...
HexOS can pick up credentials from:
...
### 8) Model scan / probe
`hexos models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
