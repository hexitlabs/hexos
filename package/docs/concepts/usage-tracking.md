...
- `/usage cost` in chats: local cost summary aggregated from HexOS session logs.
- CLI: `hexos status --usage` prints a full per-provider breakdown.
- CLI: `hexos channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
