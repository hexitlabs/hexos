...
function listConfiguredAccountIds(cfg: HexOSConfig): string[] {
  const accounts = (cfg.channels?.zalo as ZaloConfig | undefined)?.accounts;
  if (!accounts || typeof accounts !== "object") return [];
  return Object.keys(accounts).filter(Boolean);
}

export function listZaloAccountIds(cfg: HexOSConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZaloAccountId(cfg: HexOSConfig): string {
...
function mergeZaloAccountConfig(cfg: HexOSConfig, accountId: string): ZaloAccountConfig {
...
export function listEnabledZaloAccounts(cfg: HexOSConfig): ResolvedZaloAccount[] {
