...
// NOTE: This extension is intended to be bundled with HexOS.
// When running from source (tests/dev), HexOS internals live under src/.
...
export function createLlmTaskTool(api: HexOSPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "hexos-llm-task-"));
