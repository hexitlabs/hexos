...
HexOS extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
hexos channels login --channel zalouser
...
```bash
hexos message send --channel zalouser --target <threadId> --message "Hello from HexOS!"
```
...
```bash
hexos channels login --channel zalouser              # Login via QR
hexos channels login --channel zalouser --account work
hexos channels status --probe
hexos channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
hexos directory self --channel zalouser
hexos directory peers list --channel zalouser --query "name"
hexos directory groups list --channel zalouser --query "work"
hexos directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
hexos message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
hexos message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `hexos channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
hexos channels login --channel zalouser --account work
hexos message send --channel zalouser --account work --target <id> --message "Hello"
