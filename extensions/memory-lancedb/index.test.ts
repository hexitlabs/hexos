...
const liveEnabled = HAS_OPENAI_KEY && process.env.HEXOS_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "hexos-memory-test-"));
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "hexos-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
