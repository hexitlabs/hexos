...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/hexos`)
...
If the page fails to load, start the Gateway first: `hexos gateway`.
...
- Config: view/edit `~/.hexos/hexos.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. HexOS
...
```bash
hexos gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
HexOS **blocks** Control UI connections without device identity.
...
```bash
HEXOS_CONTROL_UI_BASE_PATH=/hexos/ pnpm ui:build
```
