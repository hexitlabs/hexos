---
summary: "Use Synthetic's Anthropic-compatible API in HexOS"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. HexOS registers it as the
...
```bash
hexos onboard --auth-choice synthetic-api-key
```
...
Note: HexOS's Anthropic client appends `/v1` to the base URL, so use
