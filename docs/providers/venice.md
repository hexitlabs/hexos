---
summary: "Use Venice AI privacy-focused models in HexOS"
read_when:
  - You want privacy-focused inference in HexOS
...
```bash
hexos onboard --auth-choice venice-api-key
```
...
```bash
hexos chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, HexOS shows all available Venice models. Pick based on your needs:
...
```bash
hexos models set venice/claude-opus-45
hexos models set venice/llama-3.3-70b
```

List all available models:

```bash
hexos models list | grep venice
```

## Configure via `hexos configure`

1. Run `hexos configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
HexOS automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
hexos chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
hexos chat --model venice/claude-opus-45

# Use uncensored model
hexos chat --model venice/venice-uncensored

# Use vision model with image
hexos chat --model venice/qwen3-vl-235b-a22b

# Use coding model
hexos chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `hexos models list` to see currently available models. Some models may be temporarily offline.
