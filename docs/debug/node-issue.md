...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in HexOS
---

# Node + tsx "__name is not a function" crash

## Summary
Running HexOS via Node with `tsx` fails at startup with:

```
[hexos] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `hexos status` and `gateway:watch` worked.
