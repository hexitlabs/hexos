---
summary: "First-run onboarding flow for HexOS (macOS app)"
read_when:
...
- Writes credentials to `~/.hexos/credentials/oauth.json`
...
The app can install the global `hexos` CLI via npm/pnpm so terminal
...
On the first agent run, HexOS bootstraps a workspace (default `~/clawd`):
...
```bash
hexos webhooks gmail setup --account you@gmail.com
```
