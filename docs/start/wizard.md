...
The onboarding wizard is the **recommended** way to set up HexOS on macOS,
...
(`web_fetch` works without a key). Easiest path: `hexos configure --section web`
...
1) **Existing config detection**
   - If `~/.hexos/hexos.json` exists, choose **Keep / Modify / Reset**.
   - Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset**
     (or pass `--reset`).
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `hexos doctor` before continuing.
...
2) **Model/Auth**
  ...
  - OAuth credentials live in `~/.hexos/credentials/oauth.json`; auth profiles live in `~/.hexos/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
...
5) **Channels**
  ...
  - DM security: default is pairing. First DM sends a code; approve via `hexos pairing approve <channel> <code>` or use allowlists.
...
7) **Health check**
   - Starts the Gateway (if needed) and runs `hexos health`.
   - Tip: `hexos status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `hexos agents add <name>` to create a separate agent with its own workspace,
...
- Stores it under `~/.hexos/tools/signal-cli/<version>/`.
...
Typical fields in `~/.hexos/hexos.json`:
...
`hexos agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.hexos/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.hexos/agents/<agentId>/sessions/`.
