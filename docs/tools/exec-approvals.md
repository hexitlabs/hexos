...
- **gateway host** → `hexos` process on the gateway machine
...
`~/.hexos/exec-approvals.json` directly.

CLI: `hexos approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
