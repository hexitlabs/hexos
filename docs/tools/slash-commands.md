...
- `commands.config` (default `false`) enables `/config` (reads/writes `hexos.json`).
...
- `/think <off|minimal|low|medium|high|xhigh|max|ultra>` (dynamic choices by model/provider; aliases: `/thinking`, `/t`)
...
- For full provider usage breakdown, use `hexos status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from HexOS session logs.
...
Notes:
- Overrides apply immediately to new config reads, but do **not** write to `hexos.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`hexos.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[hexos]"
