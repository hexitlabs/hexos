---
summary: "Agent tool surface for HexOS (browser, canvas, nodes, message, cron) replacing legacy `hexos-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `hexos-*` skills
---

# Tools (HexOS)

HexOS exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `hexos-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `hexos.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, HexOS logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`hexos node run`).
...
Notes:
- Requires a Brave API key (recommended: `hexos configure --section web`, or set `BRAVE_API_KEY`).
...
- Quick smoke: `hexos nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `hexos gateway` restart in-place)
