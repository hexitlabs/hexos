...
- `gateway`/`node` approvals are controlled by `~/.hexos/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  HexOS prepends `env.PATH` after profile sourcing; `tools.exec.pathPrepend` applies here too.
