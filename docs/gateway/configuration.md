---
summary: "All configuration options for ~/.hexos/hexos.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

HexOS reads an optional **JSON5** config from `~/.hexos/hexos.json` (comments + trailing commas allowed).

If the file is missing, HexOS uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
HexOS only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `hexos doctor`, `hexos logs`, `hexos health`, `hexos status`, `hexos service`, `hexos help`).
- Run `hexos doctor` to see the exact issues.
- Run `hexos doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `hexos config set`. Keep a backup of `~/.hexos/hexos.json`.
...
```bash
hexos gateway call config.get --params '{}' # capture payload.hash
hexos gateway call config.apply --params '{
...
```bash
hexos gateway call config.get --params '{}' # capture payload.hash
hexos gateway call config.patch --params '{
...
{ "$include": "/etc/hexos/base.json5" } // absolute
...
```json5
// ~/.hexos/clients/mueller/agents.json5
[
...
```json5
// ~/.hexos/clients/mueller/broadcast.json5
{
...
HexOS reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, HexOS runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `HEXOS_LOAD_SHELL_ENV=1`
- `HEXOS_SHELL_ENV_TIMEOUT_MS=15000`
...
HexOS stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.hexos/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.hexos/credentials/oauth.json` (or `$HEXOS_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.hexos/agent/*` (migrated by `hexos doctor` into `~/.hexos/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `HEXOS_OAUTH_DIR`
- Agent dir (default agent root override): `HEXOS_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, HexOS imports `oauth.json` entries into `auth-profiles.json`.

HexOS also auto-syncs OAuth tokens from external CLIs into `auth-profiles.json` (when present on the gateway host):
...
credential is a setup-token. HexOS auto-migrates older configs that used
`mode: "token"`.
...
If set, HexOS derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/hexos/hexos-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/hexos/hexos.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.hexos/credentials/whatsapp/biz
          // authDir: "~/.hexos/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `hexos doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "hexos", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.hexos/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `hexos.json`).
...
HexOS starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
HexOS starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `hexos plugins install @hexos/mattermost` (or `./extensions/mattermost` from a git checkout).
...
HexOS starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
HexOS spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, HexOS
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/hexos" } }
}
```
...
When a file exceeds this limit, HexOS logs a warning and injects a truncated
...
message envelopes). If unset, HexOS uses the host timezone at runtime.
...
`[hexos]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[hexos]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[hexos]"`, HexOS will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, HexOS generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.hexos/settings/tts.json",
      elevenlabs: {
...
HexOS also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, HexOS currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `hexos configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:hexos`: all built-in HexOS tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.hexos/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "hexos-sandbox-browser:bookworm-slim",
          containerPrefix: "hexos-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
HexOS uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.hexos/agents/<agentId>/agent/models.json` or by defining the same schema inside your
HexOS config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, HexOS writes/merges a `models.json` into
`~/.hexos/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. HexOS uses
...
- Shortcut: `hexos onboard --auth-choice opencode-zen`.
...
Shortcut: `hexos onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `hexos onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `hexos onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `hexos onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `hexos onboard --auth-choice minimax-api`.
...
- Override the agent config root with `HEXOS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.hexos/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.hexos/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.hexos/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, HexOS stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.hexos/skills` (workspace skills
...
from `~/.hexos/extensions`, `<workspace>/.hexos/extensions`, plus any
...
HexOS can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control server.
...
- Note: the control server is started by the running gateway (HexOS.app menubar, or `hexos gateway`).
...
- When a connection comes from one of these IPs, HexOS uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `hexos gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `HEXOS_GATEWAY_PORT` > `gateway.port` > default `18789`.
- Non-loopback binds (`lan`/`tailnet`/`auto`) require auth. Use `gateway.auth.token` (or `HEXOS_GATEWAY_TOKEN`).
...
- `gateway.auth.password` can be set here, or via `HEXOS_GATEWAY_PASSWORD` (recommended).
...
macOS app behavior:
- HexOS.app watches `~/.hexos/hexos.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.hexos/hexos.json` (or `HEXOS_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.hexos/hexos.json` (or `HEXOS_CONFIG_PATH`)
...
- `HEXOS_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `hexos --dev …` → uses `~/.hexos-dev` + shifts ports from base `19001`
- `hexos --profile <name> …` → uses `~/.hexos-<name>` (port via config/env/flags)
...
Example:
```bash
HEXOS_CONFIG_PATH=~/.hexos/a.json \
HEXOS_STATE_DIR=~/.hexos-a \
hexos gateway --port 19001
```
...
Gmail helper config (used by `hexos webhooks gmail setup` / `run`):
...
- Set `HEXOS_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, HexOS defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__hexos/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__hexos__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.hexos/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.hexos/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.hexos/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_hexos-bridge._tcp` under `~/.hexos/dns/` using the standard discovery domain `hexos.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `hexos.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `hexos.internal` via that server
