...
- During heartbeat runs, HexOS treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `hexos sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, HexOS skips the heartbeat run entirely (no model call).
...
```bash
hexos system event --text "Check for urgent follow-ups" --mode now
```
