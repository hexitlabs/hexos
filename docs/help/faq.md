...
HexOS supports **OpenAI Code (Codex)** via OAuth or by reusing your Codex CLI login (`~/.codex/auth.json`). The wizard can import the CLI login or run the OAuth flow and will set the default model to `openai-codex/gpt-5.6-sol` when appropriate, while seeding explicit `openai-codex` API metadata. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).
...
- **Default + switch:** set `agents.defaults.model.primary` to `openai-codex/gpt-5.6-sol`, then switch to another configured Codex model when a task requires it.
