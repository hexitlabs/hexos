...
read_when:
  - You are building a HexOS plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (hexos.plugin.json)

Every plugin **must** ship a `hexos.plugin.json` file in the **plugin root**.
HexOS uses this manifest to validate configuration **without executing plugin
