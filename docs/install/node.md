...
read_when:
  - You installed HexOS but `hexos` is “command not found”
...
HexOS’s runtime baseline is **Node 22+**.

If you can run `npm install -g hexos@latest` but later see `hexos: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
