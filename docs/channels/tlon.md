...
Tlon is a decentralized messenger built on Urbit. HexOS connects to your Urbit ship and can
...
```bash
hexos plugins install @hexos/tlon
```

Local checkout (when running from a git repo):

```bash
hexos plugins install ./extensions/tlon
```
...
Use these with `hexos message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, HexOS replies in-thread.
