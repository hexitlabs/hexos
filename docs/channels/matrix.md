...
Matrix is an open, decentralized messaging protocol. HexOS connects as a Matrix **user**
...
```bash
hexos plugins install @hexos/matrix
```

Local checkout (when running from a git repo):

```bash
hexos plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `hexos plugins install @hexos/matrix`
   - From a local checkout: `hexos plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: HexOS calls the same
     login endpoint, stores the access token in `~/.hexos/credentials/matrix/credentials.json`,
...
- On first connection, HexOS requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  HexOS logs a warning.
...
`~/.hexos/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `hexos pairing list matrix`
  - `hexos pairing approve matrix <CODE>`
...
- On startup, HexOS resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
