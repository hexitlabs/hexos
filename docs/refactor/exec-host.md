...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.hexos/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.hexos/node.json` (node id + pairing token).
...
Path: `~/.hexos/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.hexos/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.hexos/exec-approvals.sock` (0600).
