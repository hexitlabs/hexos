...
- Jobs persist under `~/.hexos/cron/` so restarts don’t lose schedules.
...
- `thinking`: Thinking level (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`; availability depends on the selected model)
...
## Storage & history
- Job store: `~/.hexos/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.hexos/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
hexos cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
hexos cron edit <jobId> --agent ops
hexos cron edit <jobId> --clear-agent
```
```
...
```bash
hexos system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`hexos system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `HEXOS_SKIP_CRON`.
