...
function formatAuthIssueHint(issue) {
    ...
    if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
        return `Run \`codex login\` (or \`${formatCliCommand("hexos configure")}\` → OpenAI Codex OAuth).`;
    }
    return `Re-auth via \`${formatCliCommand("hexos configure")}\` or \`${formatCliCommand("hexos onboard")}\`.`;
}
