...
export function formatBonjourInstanceName(displayName) {
    ...
    if (!trimmed)
        return "HexOS";
    if (/hexos/i.test(trimmed))
        return trimmed;
    return `${trimmed} (HexOS)`;
}
export function resolveBonjourCliPath(opts = {}) {
    const env = opts.env ?? process.env;
    const envPath = env.HEXOS_CLI_PATH?.trim();
    if (envPath)
        return envPath;
    ...
    const siblingCli = path.join(execDir, "hexos");
    ...
    if (isFile(distCli))
        return distCli;
    const binCli = path.join(cwd, "bin", "hexos.js");
    if (isFile(binCli))
        return binCli;
    return undefined;
}
export async function resolveTailnetDnsHint(opts) {
    const env = opts?.env ?? process.env;
    const envRaw = env.HEXOS_TAILNET_DNS?.trim();
